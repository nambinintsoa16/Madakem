...
class Comptabilite extends My_Controller
{
  ...
  public function detailProduction(){
	...
	$data = [
		"PO" => $this->input->post("po"),
		"extrusion" => $this->sachet_extrusion->get_sachet_extrusion(["EX_BC_ID" => $po]),
		"impression" => $this->sachet_impression->get_sachet_impression(["BC_ID" => $po]),
		"coupe" => $this->sachet_coupe->get_sachet_coupe(["BC_ID" => $po]),
		"qc" => $this->controle_qualite->get_controle_qualite(["C_PO" => $po]),
		"matier" => $this->sortie_matiere_premiere->get_sortie_matiere_premiere(["SM_DESTINATAIRE" => $po, "EM_TYPE" => "sortie"]),
		"matierinression" => $this->matiere_impression_use->get_matiere_impression_use(['MI_PO' => $po])
	];
	$this->load->view('Comptabilite/costing/detailProd', $data);
  }
    ...
    $tableArray = array();
				...
				if ($date == "") {
					if ($po != "") {
						...
						$date = date('Y-m');
						$extru =$this->global->get_data_joint_parameter('sachet_extrusion','commande','commande.BC_PE=sachet_extrusion.EX_BC_ID',"(BC_TYPEMATIER = '$type' AND commande.BC_ORIGINE='MADAKEM') AND  EX_DATE like '$date%'");
						$coupe =$this->global->get_data_joint_parameter('sachet_coupe','commande','commande.BC_PE=sachet_coupe.BC_ID',"(BC_TYPEMATIER = '$type' AND commande.BC_ORIGINE='MADAKEM') AND ED_DATE like '$date%'");
						$imprm =$this->global->get_data_joint_parameter('sachet_impression','commande','commande.BC_PE=sachet_impression.BC_ID',"(BC_TYPEMATIER = '$type' AND commande.BC_ORIGINE='MADAKEM') AND EI_DATE like '$date%'");
					...
					if($findate !="" ){
						...
						$extru =$this->global->get_data_joint_parameter('sachet_extrusion','commande','commande.BC_PE=sachet_extrusion.EX_BC_ID',"(commande.BC_TYPEMATIER = '$type' AND commande.BC_ORIGINE='MADAKEM') AND EX_DATE  BETWEEN '$date' AND '$findate'");
						$coupe =$this->global->get_data_joint_parameter('sachet_coupe','commande','commande.BC_PE=sachet_coupe.BC_ID',"(commande.BC_TYPEMATIER = '$type' AND commande.BC_ORIGINE='MADAKEM') AND ED_DATE  BETWEEN '$date' AND '$findate'");
						$imprm =$this->global->get_data_joint_parameter('sachet_impression','commande','commande.BC_PE=sachet_impression.BC_ID',"(commande.BC_TYPEMATIER = '$type' AND commande.BC_ORIGINE='MADAKEM') AND EI_DATE BETWEEN '$date' AND '$findate'");

					}else{
						$dateTEmp = explode("-", $date);
					    $date = $dateTEmp[0] . "-" . $dateTEmp[1];

						$extru =$this->global->get_data_joint_parameter('sachet_extrusion','commande','commande.BC_PE=sachet_extrusion.EX_BC_ID',"(commande.BC_TYPEMATIER = '$type' AND commande.BC_ORIGINE='MADAKEM') AND EX_DATE like '$date%'");
						$coupe =$this->global->get_data_joint_parameter('sachet_coupe','commande','commande.BC_PE=sachet_coupe.BC_ID',"(commande.BC_TYPEMATIER = '$type' AND commande.BC_ORIGINE='MADAKEM') AND ED_DATE like '$date%'");
						$imprm =$this->global->get_data_joint_parameter('sachet_impression','commande','commande.BC_PE=sachet_impression.BC_ID',"(commande.BC_TYPEMATIER = '$type' AND  commande.BC_ORIGINE='MADAKEM') AND EI_DATE like '$date%'");
  ...
  public function export_costing_excel(){
    ...
    $tableArray = array();
		...
		header("Content-disposition: attachment; filename=COSTING DU  : " . $date . " MADAKEM.xls");
		print $this->load->view('Comptabilite/costing/consting_containt',$reponse,true);
		exit;
  }
  public function export_costing_excel_old(){
    $this->load->model("sortie_matiere_premiere");
    $this->load->model("matiere_impression_use");
    $this->load->model("sachet_impression");
    $this->load->model("sachet_extrusion");
    ...
    $tableArray = array();
		...
		if ($date == "") {
			...
			if($findate !="" ){
				...
				$extru =$this->global->get_data_joint_parameter('sachet_extrusion','commande','commande.BC_PE=sachet_extrusion.EX_BC_ID',"(commande.BC_TYPEMATIER = '$type' AND commande.BC_ORIGINE='MADAKEM') AND EX_DATE  BETWEEN '$date' AND '$findate'");
				$coupe =$this->global->get_data_joint_parameter('sachet_coupe','commande','commande.BC_PE=sachet_coupe.BC_ID',"(commande.BC_TYPEMATIER = '$type' AND commande.BC_ORIGINE='MADAKEM') AND ED_DATE  BETWEEN '$date' AND '$findate'");
				$imprm =$this->global->get_data_joint_parameter('sachet_impression','commande','commande.BC_PE=sachet_impression.BC_ID',"c(ommande.BC_TYPEMATIER = '$type' AND commande.BC_ORIGINE='MADAKEM') AND EI_DATE like  BETWEEN '$date' AND '$findate'");
  ...
  public function matiere_liste_Data()
  {
    ...
    $datas = $this->stock_matier_premier->get_stock_matier_premier();
    $data = array();
