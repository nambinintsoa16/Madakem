...
class Commercial extends My_Controller
{
    ...
    if ($type_po == "CMTI I") {
      switch ($type) {
        ...
        case 'PE':
          ...
          break;
        case 'P':
          $methode_ok = $this->commande->insert_P_commande_madakem();
          break;
        case 'L':
          $methode_ok = $this->commande->insert_L_commande_madakem();
          break;
        default:
          $methode_ok = $this->commande->insert_pe_commande_cmtpe();
          break;
  ...
  public function sauve_commande_sachet()
  {
    ...
    $refnum_commande = 0;
    $content[18] == 'P' ? $origin ="PLASMAD": $origin = "MADAKEM";
    $method_ok = false;
    $data = [
      "BC_PE" => $content[13],
      "BC_DATE" => date('Y-m-d'),
    ...
    switch ($parametre) {
      case '1':
        $rollDim = $width;
         ...
         $hmachineEXT = ($totalMat/$VitesseMachine) * 1.062;
         $modEXT = ($totalMat/$VitesseMachine) * 0.412;
...
$totalYeild = $row / $marielYeild;
$total = $totalMat * $Prix_matier;
$hmachineEXT = ($totalMat/$VitesseMachine) * 0.412;
$modEXT = ($totalMat/$VitesseMachine) * 0.50;
$Pover = $row * 0.403;
...
$hmachineEXT = ($totalMat/ $VitesseMachine) * 1.062;
$modEXT = ($totalMat/ $VitesseMachine) * 0.412;
$hmachin = (($order*12)/15000) * 0.136;

$modeCoupe = ((($order*12)/15000)*2) *0.500;
$Pover = $totalMat * 0.403;
...
$hmachineEXT = ($totalMat/$VitesseMachine) * 1.062;
$modEXT = ($totalMat/$VitesseMachine) * 0.5;

$machinPrint =  ($totalMat/25) * 0.912;
$modPrint =  (($totalMat/25) * 2) * 0.470;

$machinecoupe = (($order*12) / 15000)*0.136;
$modeCoupe =((($order*12) / 15000)*2)*0.5;
...
$machinPrint =  ($totalYeild/2000) * 0.912;
$modPrint =  (($totalYeild/2000)) * 0.470;
$machinecoupe = (($order*12) / 15000)*0.136;
$modeCoupe =((($order*12) / 15000)*2)*0.5;
$Pover = $totalMat * 0.403;
...
$machinPrint =  ($totalMat/25) * 0.912;
$modPrint =  (($totalMat/25)*2) * 0.5;



$sealing = $totalYeild * 0.0045;

$machinecoupe = (($order*12) / 15000)*0.220;
$modeCoupe =((($order*12) / 15000)*2)*0.600;
$Pover = $totalMat * 0.403;
...
$prixavemarge = number_format($prixmarge +$prix,4);
  ...
  public function print_costing()
  {
    ...
    $data = $this->prix_commande->get_detail_prix_commande(['PB_PO' => $BC_PO]);

    $html =  $this->load->view('commercial/PreCosting/print_costing', ["data" => $data], true);
    //echo $html;
